...
import {
  ISignInContext,
  SignInContext,
} from "application/context/Admin/SignIn/SignInContext";
...
export default function SignInFormulary() {
  ...
  const { signInAdmin } = actions;
  const { loading, sucessful, error } = state.signInAdminState;
  ...
  const onSubmit = (e: any) => {
    ...
    if (formIsValid)
      signInAdmin({ email: values.email, password: values.password })(dispatch);
  };
  ...
  const handleSignInSucessful = useCallback(() => {
    ...
    window.location.href =
      window.location.origin + AdminDashboardRoutesEnum.Dashboard;
  }, [router.query]);

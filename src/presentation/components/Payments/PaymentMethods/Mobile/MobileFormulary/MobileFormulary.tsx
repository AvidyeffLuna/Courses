import {
  IPaymentMobileContext,
  PaymentMobileContext,
} from "application/context/Payments/PaymentMethods/PaymentMobile/PaymentMobileContext";
...
import { mobileValidators } from "./Validators/mobileValidator";
import { mobileFields, mobileInitialValues } from "./Validators/mobileValues";

interface IMobileFormularyProps {
  shoppingCart: IGetShoppingCartByIdResponse;
}

export default function MobileFormulary({
  shoppingCart,
}: IMobileFormularyProps) {
  const { state, actions, dispatch } =
    useContext<IPaymentMobileContext>(PaymentMobileContext);
  const { createPayment } = actions;
  const { data: transaction, loading, sucessful, error } = state.createPayment;

  const router = useRouter();

  const [fields, setFields] = useState<IFieldValue[]>(mobileFields);
  const [values, setValues] = useState(mobileInitialValues);
  const [hasError, setHasError] = useState<ISaleFailure | null>(null);
  const [picture, setPicture] = useState<IFile>({} as IFile);
  const [totalAmount] = useState(shoppingCart.data.paidSummary.totalPaid);
  ...
  const onFieldValidated = (
    ...
    const handleFieldValidator = mobileValidators[name];
  ...
  const onSubmit = (e: any) => {
    ...
    if (formIsValid) {
      ...
      const createSale: ISale = {
        ...
        phoneOperatorCode: values.phoneOperatorCode,

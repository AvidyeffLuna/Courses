...
export default function DeliverableView() {
  ...
  if (!deliverable.deliverableId && sucessful) {
    return (
      <div className="mt-5">
        <ErrorMessage
          title="No se ha encontrado la entrega o respuesta del profesor"
          description="No hemos podido encontrar la entrega o respuesta del profesor, puede que haya sido eliminado"
  ...
  if (!deliverable.deliverableId && !sucessful) {
    <div className="mt-5">
      <ErrorMessage
        ...
        description="Algo ha salido mal al intentar obtener la entrega o respuesta del profesor. Intentalo nuevamente."
        retry={getDeliverableByIdDispatch}
      />
    </div>;
  }
